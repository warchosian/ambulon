#!/usr/bin/env python3
"""
Outil de déplacement de fichiers avec pattern.
Usage: python move_files.py <source_dir> <dest_dir> [--pattern "*.md"]
"""
...
def move_files(source_dir: str, dest_dir: str, pattern: str = "*", dry_run: bool = False):
    """
    ...
    Args:
        ...
        dry_run: Simulation sans déplacement
    """
    ...
    for f in src_path.glob(pattern):
        if f.is_file():
            ...
            if dry_run:
                print(f"[DRY-RUN] Déplacement: {f} -> {dest_file}")
            else:
                print(f"Déplacement: {f} -> {dest_file}")
                shutil.move(str(f), str(dest_file))
            moved.append(f)
    
    print(f"\n{len(moved)} fichier(s) déplacé(s)")
    return 0
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Déplace des fichiers")
    ...
    sys.exit(move_files(args.source_dir, args.dest_dir, args.pattern, args.dry_run))
