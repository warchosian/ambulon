...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Flattens a nested Markdown directory structure into a single directory.",
...
Examples:
  ...
  ambulon flatten-md /path/to/nested-md

  # With custom output directory
  ambulon flatten-md /path/to/nested-md --output /path/to/flat-md

  # With configuration file
  ambulon flatten-md /path/to/nested-md -o output-dir --config config/processing.yaml
        """
    )
    ...
    parser.add_argument(
        "source_dir",
        type=Path,
        help="Source directory with nested Markdown files."
    )
    ...
    try:
        ...
        logger.error(f"Error during Markdown flattening: {e}", exc_info=True)
        return 1
