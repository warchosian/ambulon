...
def main(argv=None):
    ...
    api_token = args.token or config.get('piag', {}).get('rag', {}).get('security', {}).get('token') or os.getenv(config.get('piag', {}).get('rag', {}).get('security', {}).get('token_env_var', 'PIAG_RAG_API_TOKEN'))
    if not api_token:
