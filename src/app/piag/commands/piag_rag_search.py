...
def main(argv=None):
    ...
    parser.add_argument("--timeout", help="Timeout HTTP (défaut: 120s). Accepte: 10000, 10000ms, 10s, 2m")
    ...
    parser.add_argument("-o", "--output", help="Fichier de sortie pour sauvegarder les résultats JSON (compatible avec piag-chat-query).")
    ...
    if args.debug:
        config.setdefault('piag', {}).setdefault('rag', {}).setdefault('logging', {}).update({'enable_debug': True, 'log_requests': True, 'log_responses': True})

    # Hiérarchie de configuration
    query = args.query or config.get('search', {}).get('query') or os.getenv('PIAG_RAG_SEARCH_QUERY')
    api_token = args.token or config.get('piag', {}).get('rag', {}).get('security', {}).get('token') or os.getenv('PIAG_RAG_API_TOKEN')
    project_id = args.project_id or config.get('piag', {}).get('rag', {}).get('project', {}).get('project_id') or os.getenv('PIAG_RAG_PROJECT_ID')
    base_url = args.base_url or config.get('piag', {}).get('rag', {}).get('api', {}).get('base_url') or os.getenv('PIAG_RAG_BASE_URL')
    ...
    timeout_value = args.timeout or config.get('piag', {}).get('rag', {}).get('api', {}).get('timeout') or os.getenv('PIAG_RAG_TIMEOUT') or '120s'
    try:
        timeout = parse_timeout(timeout_value)  # Convertit en millisecondes
    except ValueError as e:
        print(f"Erreur timeout: {e}", file=sys.stderr)
        return 1

    max_retries = args.max_retries or config.get('piag', {}).get('rag', {}).get('api', {}).get('max_retries') or os.getenv('PIAG_RAG_MAX_RETRIES') or 3
    ...
    collection_name_single = args.collection_name or os.getenv('PIAG_RAG_COLLECTION_NAME') or config.get('piag', {}).get('rag', {}).get('project', {}).get('collection_name')
    collection_id_single = args.collection_id or os.getenv('PIAG_RAG_COLLECTION_ID') or config.get('piag', {}).get('rag', {}).get('project', {}).get('collection_id')
    collection_name_list_multi = args.collection_name_list or os.getenv('PIAG_RAG_COLLECTION_NAME_LIST') or config.get('piag', {}).get('rag', {}).get('project', {}).get('collection_name_list')
    collection_id_list_multi = args.collection_id_list or os.getenv('PIAG_RAG_COLLECTION_ID_LIST') or config.get('piag', {}).get('rag', {}).get('project', {}).get('collection_id_list')
    ...
    try:
        ...
        # Si --output est spécifié, sauvegarder dans un fichier JSON
        if args.output:
            ...
            num_chunks = len(result.get('chunks', result)) if isinstance(result, dict) else len(result)
            print(f"   {num_chunks} chunk(s) récupéré(s)", file=sys.stderr)
            return 0
