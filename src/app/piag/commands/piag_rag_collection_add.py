...
def main(argv=None):
    ...
    Exemple d'utilisation:
        python -m app.piag.commands.piag_rag_collection_add --project-id <id> --collection "Test" --description "Test" --token <token>

    Ou avec variable d'environnement:
        export PIAG_RAG_API_TOKEN=<token>
        python -m app.piag.commands.piag_rag_collection_add --project-id <id> --collection "Test" --description "Test"
    """
    ...
    if not project_id:
        project_id = config.get('piag', {}).get('rag', {}).get('project', {}).get('project_id')  # YAML (priorité 2)
    if not project_id:
    ...
    if not name:
        name = config.get('piag', {}).get('rag', {}).get('project', {}).get('collection_name')  # YAML (priorité 2)
    if not name:
    ...
    if not description:
        description = config.get('piag', {}).get('rag', {}).get('project', {}).get('description')  # YAML (priorité 2)
    if not description:
    ...
    if not api_token:
        api_token = config.get('piag', {}).get('rag', {}).get('security', {}).get('token')  # YAML (priorité 2) - NON RECOMMANDÉ
    if not api_token:
