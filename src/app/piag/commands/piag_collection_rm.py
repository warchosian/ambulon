"""Module CLI pour supprimer une collection RAG PIAG."""
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Supprime une collection RAG PIAG.")
    ...
    try:
        ...
        if not args.force:
            confirm = input(f"Voulez-vous vraiment supprimer la collection '{resolved_collection_id}' ? (oui/non): ")
            if confirm.lower() != 'oui':
                print("Suppression annulée")
                return 0
        
        result = client.delete_collection(resolved_collection_id)
