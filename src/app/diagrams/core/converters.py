...
def _get_plantuml_jar_path(cli_jar_path: Optional[str] = None) -> Optional[str]:
    ...
    """
    if cli_jar_path and os.path.exists(cli_jar_path):
        return cli_jar_path
...
def _convert_plantuml_with_jar(
    ...
    if not _java_available():
        return ConversionResult(
            success=False,
            error_message="Java not available or version too old"
        )
    ...
    try:
        ...
        if result.returncode == 0 and os.path.exists(temp_output):
            ...
            error = result.stderr.decode('utf-8', errors='replace') if result.stderr else "Unknown error"
            if os.path.exists(temp_output):
                os.unlink(temp_output)
            return ConversionResult(
                success=False,
                error_message=f"PlantUML JAR failed: {error}"
            )
            
    except subprocess.TimeoutExpired:
        return ConversionResult(
            success=False,
            error_message="PlantUML JAR conversion timed out"
        )
    except Exception as e:
        return ConversionResult(
            success=False,
            error_message=f"PlantUML JAR error: {e}"
        )
...
def _convert_plantuml_with_kroki(
    ...
    try:
        ...
        return ConversionResult(
            success=False,
            error_message=f"Kroki conversion error: {e}"
        )
...
def convert_mermaid(
    ...
    try:
        ...
        return ConversionResult(
            success=False,
            error_message=f"Mermaid conversion error: {e}"
        )
...
def _convert_graphviz_with_dot(
    dot_code: str,
    timeout: int = 30
) -> ConversionResult:
    """
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return ConversionResult(
                success=False,
                error_message=f"Graphviz failed: {error}"
            )
            
    except FileNotFoundError:
        return ConversionResult(
            success=False,
            error_message="Graphviz 'dot' command not found"
        )
    except subprocess.TimeoutExpired:
        return ConversionResult(
            success=False,
            error_message="Graphviz conversion timed out"
        )
    except Exception as e:
        return ConversionResult(
            success=False,
            error_message=f"Graphviz error: {e}"
        )


def _convert_graphviz_with_kroki(
    dot_code: str,
    timeout: int = 30
) -> ConversionResult:
    """
    ...
    try:
        ...
        return ConversionResult(
            success=False,
            error_message=f"Kroki Graphviz error: {e}"
        )
