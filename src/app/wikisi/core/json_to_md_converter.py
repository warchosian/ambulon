"""
Core logic for converting JSON data to optimal Markdown format for Ambulon.
Contains helper functions for formatting, filtering, and conversion.
"""
...
def convert_app_to_markdown(app: Dict, verbose: bool = False) -> str:
    ...
    domaines = get_field(app, "domaines et sous domaines", "domaines et sous-domaine", "domaines", "domains")
    ...
    prec_obligation = get_field(app, "precisions sur le obligation", "précisions sur l'obligation")
    ...
    processed_keys = {
        ...
        "dict", "dict disponibilite", "dict - disponibilité",
        "dict integrite", "dict - integrité", "dict - intégrité",
        "dict tracabilite", "dict - traçabilité", "dict - tracabilité",
        "dict confidentialite", "dict- confidentialité", "dict - confidentialité",
        "traitement de donnees a caractere personnel", "traitement de données à caractère personnel",
        "dacp traitees", "dacp traitées",
        "categories particulieres de dacp", "catégories particulières de dacp",
        "mode de collecte des informations traitees", "mode de collecte des informations traitées",
        "destinataires des donnees", "destinataires des données",
        "base juridique du traitement",
        "finalites du traitement", "finalités du traitement",
        "categories particulieres de finalites", "catégories particulières de finalités",
        "necessite reglementaire d une aipd", "nécessité réglementaire d'une aipd",
        "commentaires sur la confidentialite", "commentaires sur la confidentialité",
        "gravites d impacts", "gravités d'impacts",
        "gravites d impacts desorganisation interne ou externe", "gravités d'impacts - désorganisation interne ou externe",
        "gravites d impacts financier", "gravités d'impacts - financier",
        "gravites d impacts juridique", "gravités d'impacts - juridique",
        "gravites d impacts personnes", "gravités d'impacts - personnes",
        "gravites d impacts politique image", "gravités d'impacts - politique image",
        "si a enjeux", "si à enjeux",
        "etudes des besoins de securite et des risques", "études des besoins de sécurité et des risques",
        "moa habilitee ssi", "moa habilitée ssi",
        "contacts ssi",
        "approche produit",
        "developpement agile", "développement agile",
        "proposition de valeur",
        "obligation",
        "precisions sur le obligation", "précisions sur l'obligation",
        "application figurant dans les cartographie d urbanisme",
        "participe a la cartographie numerique pour la transition ecologique",
        "evolution technologique", "évolution technologique",
        "evolution du contenu", "évolution contenu",
        "evolution des conditions d acces", "évolution accès",
        "evolution de l usage", "évolution usage",
        "utilisateurs", "users",
        "utilisateurs actifs par mois",
...
def process_parkjson2md_logic(
    ...
    """
    Core logic to process JSON file to optimal Markdown format (parkjson2md).
    ...
    if not input_file.exists():
        logger.error(f"Error: '{input_file}' does not exist.")
        return 1, None
    ...
    try:
        ...
        if split_dir:
            ...
            logger.info(f"[INFO] Converting {len(apps)} application(s) to separate Markdown files...")
            ...
            for i, app_data in enumerate(apps): # Renamed 'app' to 'app_data' to avoid conflict with 'app' variable in typer
                ...
                filename = f"{input_file.stem}_{safe_app_name}.md"
                file_path = split_dir / filename

                # Generate markdown for this application
                app_md = convert_app_to_markdown(app_data) # Removed verbose from here
            ...
            logger.info(f"[SUCCESS] {files_created} Markdown files created in {split_dir}")
        ...
        if output_file is None:
            if used_filter and tag_parts:
                ...
                output_path = input_file.with_name(f"{input_file.stem}_{tag_suffix}.md")
            else:
        ...
        logger.info(f"[INFO] Converting {len(apps)} application(s) to Markdown (parkjson2md format)...")
        logger.info(f"[INFO] Input:  {input_file}")
        ...
        for i, app_data in enumerate(apps): # Renamed 'app' to 'app_data'
            if (i + 1) % 100 == 0:
                logger.debug(f"[INFO] Processed {i + 1}/{len(apps)} applications...")

            md_lines.append(convert_app_to_markdown(app_data)) # Removed verbose from here
            md_lines.append("---")
            md_lines.append("")
        ...
        logger.info(f"[SUCCESS] Markdown file created: {output_path}")
        return 0, output_path

    except Exception as e:
        logger.error(f"Error: Conversion failed: {e}", exc_info=True)
        return 1, None
