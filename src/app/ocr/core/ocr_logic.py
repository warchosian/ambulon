...
# Imports optionnels
try:
    import pytesseract  # noqa: F401  (probe d'import pour TESSERACT_AVAILABLE)
...
def perform_ocr(
    input_path: Path, language: str = "fra", output_path: Path = None
) -> Dict[str, Any]:
    """
    ...
    if not input_path.exists():
        return {"success": False, "error": f"Chemin non trouvé : {input_path}"}
    ...
    try:
        ...
        if not tesseract_enabled:
            ...
            if python_alt and fallback_enabled:
                logging.info(
                    f"[FALLBACK] Tesseract désactivé, utilisation de l'alternative Python: {python_alt}"
                )
                print(f"[FALLBACK] tesseract désactivé → utilisation de {python_alt}")
                return _perform_ocr_python(input_path, language, output_path)
            else:
                logging.error(
                    "[ERREUR] Tesseract désactivé et aucune alternative Python configurée"
                )
                return {
                    "success": False,
        ...
        logging.info("[OCR] Démarrage de l'OCR avec Tesseract")
        ...
        cmd = [
            ...
            language,
            "--psm",
            "3",  # Page segmentation mode: Fully automatic page segmentation
            "--oem",
            "3",  # OCR Engine Mode: Default, based on what is available
        ]

        logging.info(f"[OCR] Commande : {' '.join(cmd)}")
        print("Lancement de l'OCR avec Tesseract...")
...
def _perform_ocr_python(
    image_file: Path, language: str = "fra", output_file: Path = None
) -> Dict[str, Any]:
    """
    ...
    try:
        if not TESSERACT_AVAILABLE:
            ...
            return {"success": False, "error": "pytesseract n'est pas disponible"}
        ...
        logging.info("[OCR] Démarrage de l'OCR avec pytesseract")
        logging.info(f"[OCR] Langue : {language}")
        ...
        if ocr_output_file.exists():
            ocr_size = ocr_output_file.stat().st_size
            logging.info(
                f"[OCR] OCR réussi avec pytesseract : {ocr_output_file} ({ocr_size} octets)"
            )
            print(
                f"Fichier OCR créé avec pytesseract : {format_output_path(ocr_output_file)}"
            )
            print(f"Taille du fichier OCR : {ocr_size} octets")
            return {
                "success": True,
...
def _process_directory_ocr(
    directory: Path, language: str = "fra", output_dir: Path = None
) -> Dict[str, Any]:
    """
...
def _process_pdf_ocr(
    pdf_file: Path, language: str = "fra", output_file: Path = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for page_num in range(pages_count):
            ...
            if text.strip():
                ...
                try:
                    ...
                    ocr_result = perform_ocr_single_file(
                        temp_image, language, temp_output
                    )
        ...
        return {"success": False, "error": f"Erreur lors du traitement PDF: {str(e)}"}


def perform_ocr_single_file(
    image_file: Path, language: str = "fra", output_file: Path = None
) -> Dict[str, Any]:
    """
    ...
    if not image_file.exists():
        return {"success": False, "error": f"Fichier image non trouvé : {image_file}"}
...
def _table_to_markdown(table: List[List[str]]) -> str:
    ...
    for row in table[1:]:
        clean_row = [
            clean_cell(cell, preserve_breaks=not is_title_table) for cell in row
        ]
        if any(clean_row):
...
def _process_pdf_ocr(
    pdf_file: Path, language: str = "fra", output_file: Path = None, mode: str = "auto"
) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        language: Langue pour l'OCR
        output_file: Fichier de sortie (optionnel)
        mode: 'auto' (texte + OCR fallback si texte court),
              'text' (extraction texte seule, jamais d'OCR),
              'ocr' (OCR sur image rendue, ignore la couche texte),
              'merged' (texte natif + OCR en complément sur chaque page,
                       maximise les infos extraites)
    ...
    try:
        ...
        logging.info(f"[OCR] Traitement PDF (mode={mode}) : {pdf_file}")
        ...
        with pdfplumber.open(pdf_file) as pdf:
            ...
            for page_num, page in enumerate(pdf.pages):
                ...
                # En mode 'ocr', on ignore la couche texte du PDF
                if mode == "ocr":
                    text = None
                    tables = []
                else:
                ...
                if text and table_lines:
                    ...
                    for line in text_lines:
                        ...
                        for t_line in table_lines:
                            if line_clean and (
                                line_clean.startswith(t_line)
                                or t_line.startswith(line_clean[:50])
                            ):
                                is_in_table = True
                                break
                            if line_simplified and (
                                line_simplified.startswith(t_line)
                                or t_line.startswith(line_simplified[:50])
                            ):
                                is_in_table = True
                                break
                ...
                if tables:
                    logging.info(
                        f"[OCR] Page {page_num + 1}: {len(tables)} tableau(x) détecté(s)"
                    )
                    for table in tables:
                ...
                # Heuristique : si peu de texte extractible et pas de tableau, la page
                # contient probablement du contenu image non couvert par la couche texte
                MIN_TEXT_THRESHOLD = 50
                extracted_len = sum(len(c) for c in page_content)
                if mode == "text":
                    needs_ocr_fallback = False
                elif mode == "ocr":
                    needs_ocr_fallback = True
                elif mode == "merged":
                    # Toujours faire OCR en complément, même si texte natif présent
                    needs_ocr_fallback = True
                else:  # auto
                    needs_ocr_fallback = (not page_content) or (
                        extracted_len < MIN_TEXT_THRESHOLD and not tables
                    )

                # Ajouter le contenu textuel extrait s'il existe
                if page_content:
                    all_text.append(f"\n--- Page {page_num + 1} ---\n")
                    all_text.extend(page_content)

                # Compléter par OCR si la page est vide ou quasi-vide
                if needs_ocr_fallback:
                    reason = (
                        "aucun contenu détecté"
                        if not page_content
                        else f"texte trop court ({extracted_len} car.)"
                    )
                    logging.info(
                        f"[OCR] Page {page_num + 1}: {reason}, conversion en image"
                    )
                    try:
                        import fitz
        ...
        return {"success": False, "error": f"Dépendance manquante: {str(e)}"}
    except Exception as e:
        return {"success": False, "error": f"Erreur lors du traitement PDF: {str(e)}"}
...
def process_multiple_files(
    ...
    for file_path in matching_files:
        ...
        try:
            ...
            if file_path.is_dir():
                ocr_result = _process_directory_ocr(
                    file_path, language, output_file.parent if output_file else None
                )
            else:
...
Langues supportées (exemples):
  ...
  fra+eng - Français et Anglais combinés
    ...
    parser.add_argument(
        "-o", "--output", help="Fichier ou répertoire de sortie (optionnel)"
    )
    parser.add_argument(
        "-l", "--lang", default="fra", help="Langue pour l'OCR (défaut: fra)"
    )

    # Options de traitement
    parser.add_argument(
        "--batch", action="store_true", help="Mode lot pour traiter plusieurs fichiers"
    )
    parser.add_argument(
        "--psm",
        type=int,
    ...
    parser.add_argument(
        "--format",
        choices=["txt", "pdf", "hocr"],
        default="txt",
        help="Format de sortie (défaut: txt)",
    )
    parser.add_argument(
        "--confidence", action="store_true", help="Inclure les scores de confiance"
    )
    ...
    parser.add_argument(
        "--force", action="store_true", help="Écraser les fichiers existants"
    )
    ...
    logging.info("[DÉMARRAGE] Démarrage du module OCR")
    ...
    if input_path.is_dir():
        ...
        try:
            result = _process_directory_ocr(
                directory=input_path, language=args.lang, output_dir=output_dir
            )
        except Exception as e:
        ...
        try:
            result = perform_ocr_single_file(
                image_file=input_path, language=args.lang, output_file=output_file
            )
        except Exception as e:
            ...
            if args.quiet:
                ...
                try:
                    ...
                    for file_info in result["processed_files"]:
                        try:
                            input_path = str(
                                file_info["input_file"].relative_to(Path.cwd())
                            )
                            output_path = str(
                                file_info["output_file"].relative_to(Path.cwd())
                            )
                        except ValueError:
                        ...
                        content_lines.append(
                            f"[blue]  → [/blue]{output_path} ({file_info['file_size']} octets)"
                        )
                    ...
                    print(
                        f"OCR en lot réussi : {result['successful_files']}/{result['total_files']}"
                    )
                    ...
                    for file_info in result["processed_files"]:
                        ...
                        print(
                            f"    → {file_info['output_file']} ({file_info['file_size']} octets)"
                        )
            ...
            if args.quiet:
                ...
                try:
                    ...
                    if result.get("method"):
                        content_lines.append(
                            f"[cyan]Méthode :[/cyan] {result['method']}"
                        )
