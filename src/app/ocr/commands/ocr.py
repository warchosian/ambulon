...
from app.ocr.core.ocr_logic import _process_pdf_ocr, perform_ocr_single_file, process_multiple_files
...
DEFAULT_CONFIG = {
    ...
    "tools": {
        ...
        "tesseract_command": os.getenv(
            "AMBULON_TESSERACT_COMMAND", os.getenv("TESSERACT_COMMAND", "tesseract")
        ),
        "tesseract_enabled": os.getenv(
            "AMBULON_TESSERACT_ENABLED", os.getenv("TESSERACT_ENABLED", "True")
        ).lower()
        == "true",
        "tesseract_python_alternative": os.getenv(
            "AMBULON_TESSERACT_PYTHON_ALTERNATIVE",
            os.getenv("TESSERACT_PYTHON_ALTERNATIVE", "False"),
        ).lower()
        == "true",
        "tesseract_fallback_enabled": os.getenv(
            "AMBULON_TESSERACT_FALLBACK_ENABLED",
            os.getenv("TESSERACT_FALLBACK_ENABLED", "False"),
        ).lower()
        == "true",
        "tesseract_timeout": os.getenv(
            "AMBULON_TESSERACT_TIMEOUT", os.getenv("TESSERACT_TIMEOUT", "60")
        ),
    },
}
...
Environment variables:
  ...
  TESSERACT_ENABLED     Enable Tesseract (True/False)
    ...
    parser.add_argument(
        "input",
        type=str,
        help='Input file, directory, or glob pattern (e.g., image.jpg, folder/, "*.png").',
    )
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
        help="Output file or directory (default: same folder as input, .txt extension).",
    )
    parser.add_argument(
        "-c",
        "--config",
        type=Path,
        help="Path to a YAML configuration file (e.g., config/ocr.yaml).",
    )
    ...
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="Suppress most output messages."
    )

    # OCR specific options
    parser.add_argument(
        "-l", "--lang", type=str, help="Language for OCR (e.g., fra, eng, fra+eng)."
    )
    ...
    # PDF processing mode (only relevant for PDF inputs)
    parser.add_argument(
        "--mode",
        type=str,
        choices=["auto", "text", "ocr", "merged"],
        default="auto",
        help="PDF mode: 'auto' (texte + OCR fallback si texte court), "
        "'text' (extraction texte seule), 'ocr' (OCR seul sur image rendue), "
        "'merged' (texte + OCR systématique, max d'infos). Défaut: auto.",
    )
    parser.add_argument(
        "--all-modes",
        "--dual",
        dest="all_modes",
        action="store_true",
        help="Génère 3 fichiers MD : <name>.text.md (texte extractible), "
        "<name>.ocr.md (OCR sur image rendue) et <name>.merged.md "
        "(texte + OCR combinés, max d'infos). Pour PDF uniquement. "
        "(--dual conservé comme alias.)",
    )
    ...
    final_language = (
        ...
        else ocr_config.get("language", DEFAULT_CONFIG["ocr"]["language"])
    )
    final_psm = (
        args.psm
        if args.psm is not None
        else ocr_config.get("psm", DEFAULT_CONFIG["ocr"]["psm"])
    )
    final_oem = (
        args.oem
        if args.oem is not None
        else ocr_config.get("oem", DEFAULT_CONFIG["ocr"]["oem"])
    )
    ...
    if input_path.is_dir() or has_wildcards:
        ...
        logger.info(
            f"Mode: Processing multiple files from pattern/directory: {args.input}"
        )
        ...
        if input_path.suffix.lower() == ".pdf":
            if args.all_modes:
                # Génère 3 MD : <name>.text.md, <name>.ocr.md, <name>.merged.md
                base = input_path.with_suffix("")
                text_out = base.parent / f"{base.name}.text.md"
                ocr_out = base.parent / f"{base.name}.ocr.md"
                merged_out = base.parent / f"{base.name}.merged.md"
                generated_files = []
                for sub_mode, sub_out in [
                    ("text", text_out),
                    ("ocr", ocr_out),
                    ("merged", merged_out),
                ]:
                    result = _process_pdf_ocr(
                        input_path, final_language, sub_out, mode=sub_mode
                    )
                    if result.get("success") and result.get("output_file"):
                        generated_files.append(Path(result["output_file"]))
            else:
                ...
                result = _process_pdf_ocr(
                    input_path, final_language, output_file, mode=args.mode
                )
                if result.get("success") and result.get("output_file"):
                    generated_files = [Path(result["output_file"])]
        else:
            result_file = perform_ocr_single_file(
                image_file=input_path, language=final_language, output_file=args.output
            )
            if result_file:
                generated_files = [result_file]
    else:
        logger.error(
            f"Input '{args.input}' is not a valid file, directory or glob pattern."
        )
        return 1

    if generated_files:
        logger.info(
            f"OCR operation successful. Generated {len(generated_files)} file(s)."
        )
