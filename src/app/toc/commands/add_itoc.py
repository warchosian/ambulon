"""
CLI command to add back-to-TOC (inverse TOC) links to Markdown or HTML files.
Auto-detects file type and calls appropriate handler.
"""
...
def main(argv=None):
    """
    Entry point for add-itoc command (inverse TOC links).
    Auto-detects file type (.md or .html) and processes accordingly.
    ...
    parser = argparse.ArgumentParser(
        description="""
        Adds inverse TOC navigation links (↑) after each heading in a Markdown or HTML file.
        These links allow readers to quickly jump back to the table of contents.

        Auto-detects file type based on extension (.md or .html).
    ...
    parser.add_argument("input_file", type=Path, help="Path to input file (Markdown or HTML).")
    parser.add_argument("--output", "-o", type=Path, help="Output file path (default: <input>-itoc.<ext>).")
    ...
    setup_logging(level=log_level, log_file_prefix="add_itoc")
    logger.info("[START] Starting add-itoc module.")
    ...
    if file_extension == '.md':
        return _handle_markdown(args)
    elif file_extension in ['.html', '.htm']:
        return _handle_html(args)
    else:
        logger.error(f"Error: Unsupported file type '{file_extension}'. Only .md and .html files are supported.")
        return 1


def _handle_markdown(args):
    """Handle Markdown file processing."""
    logger.info("Detected Markdown file, processing...")
...
def _handle_html(args):
    """Handle HTML file processing."""
    logger.warning("HTML file detected.")
    logger.error("Error: Adding inverse TOC links to HTML files is not supported by this command.")
    logger.info("For HTML files, back-to-TOC links should be added during MD to HTML conversion:")
    logger.info("  ambulon md2html document.md --toc-backlinks")
    return 1
