...
def show_help():
    ...
    print(
        "  init                  Générer les fichiers de configuration (piag, gitlab, wikisi)"
    )
    ...
    print(
        "  vscode-install        Installer les extensions recommandées pour diagrammes"
    )
    ...
    print(
        "  llm                   Générer des documents via API LLM (Kimi, ChatGPT, Claude)"
    )
    print("  llm-filter            Filtrer et réduire les gros fichiers code.md")
    print("  llm-summarize         Résumer intelligemment les fichiers code.md via LLM")
    print("  llm-plantuml2mermaid  Convertir diagrammes PlantUML en Mermaid")
    print(
        "  llm-generate-docs     Générer des docs depuis les prompts pour toutes les applications"
    )
    print("  llm-migrate-ollama    Migrer les configurations Ollama locales")
    print()
    ...
    print(
        "  wikisi-extract-apps   Extraire les applications désignées en JSON et MD individuels"
    )
    ...
    print(
        "  wikisi-sync-api       Synchroniser les données (énumérations, applications) depuis l'API WikiSI"
    )
    print()
    ...
    print(
        "  add-itoc4md           Ajouter des liens retour (iTOC) vers lignes TOC specifiques"
    )
    print("  check-toc4md          Verifier si une TOC existe dans un fichier Markdown")
    print(
        "  check-itoc4md         Verifier si des liens iTOC existent dans un fichier Markdown"
    )
    ...
    print(
        "  augment               Augmenter HTML avec navigation interactive (anchors, TOC)"
    )
    ...
    print(
        "    piag-rag-create                 Créer un RAG complet (collection + upload)"
    )
    ...
    print(
        "    piag-rag-then-chat run          Pipeline complet (4 étapes en une commande)"
    )
    ...
    print(
        "  ambulon piag-rag-create --collection-name 'MonRAG' --desc 'Docs'         Collection vide"
    )
    print(
        "  ambulon piag-rag-create --collection-name 'MonRAG' --directory ./docs    Avec documents"
    )
    print(
        "  ambulon piag-rag-collection-list                                         Lister collections"
    )
    print(
        "  ambulon piag-rag-doc-list --collection-name 'MonRAG'                     Lister documents"
    )
    print(
        "  ambulon piag-rag-search --collection-name 'MonRAG' --query 'question'    Rechercher"
    )
    print(
        "  ambulon piag-chat-query --collection-name 'MonRAG' --query 'question'    Chat avec RAG"
    )
    print()
...
def handle_config_command():
    ...
    if subcommand == "export":
        try:
            ...
            output_file = (
                Path(sys.argv[3]) if len(sys.argv) > 3 else Path("mcp-config.json")
            )
        ...
        try:
            if assistant == "all":
                print(
                    "Installation de la configuration MCP pour tous les assistants..."
                )
                print()
        ...
        for assistant, info in status.items():
            print(f"\n{assistant.upper()}:")
            print(
                f"  Repertoire: {'OK' if info['directory_exists'] else 'NON'} {Path(info['config_path']).parent}"
            )
            print(
                f"  Config:     {'OK' if info['config_exists'] else 'NON'} {info['config_path']}"
            )
            print(
                f"  Ambulon:    {'OK' if info['ambulon_configured'] else 'NON'} {'Configure' if info['ambulon_configured'] else 'Non configure'}"
            )
        ...
        print(
            f"  Outils:     {'OK' if test_results['tools_available'] else 'NON'} ({test_results['tools_count']} disponibles)"
        )
        ...
        if test_module == "all":
            ...
            if test_module == "mcp":
                ...
                if Path(integration_file).exists():
                    ...
                    return max(
                        result1.returncode if "result1" in locals() else 0,
                        result2.returncode,
                    )
                else:
                ...
                if test_script.exists():
                    result = subprocess.run(
                        [sys.executable, str(test_script)], cwd=str(Path.cwd())
                    )
                    return result.returncode
                else:
...
def handle_rag_module(module_name: str):
    ...
    command_to_import = {
        ...
        # Collections RAG
        "piag-rag-collection-add": (
            "app.piag.commands.piag_rag_collection_add",
            "main",
        ),
        "piag-rag-collection-list": (
            "app.piag.commands.piag_rag_collection_list",
            "main",
        ),
        "piag-rag-collection-get": (
            "app.piag.commands.piag_rag_collection_get",
            "main",
        ),
        "piag-rag-collection-update": (
            "app.piag.commands.piag_rag_collection_update",
            "main",
        ),
    ...
    try:
        ...
        print(
            f"Erreur lors de l'exécution du module {module_name}: {e}", file=sys.stderr
        )
        import traceback
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "html2md":
            ...
            if len(sys.argv) < 3:
                print(
                    "Usage: ambulon html2md <input.html> [-o <output.md>] [--verbose]"
                )
                return 1
            ...
            if len(sys.argv) < 3 or sys.argv[2] in ["-h", "--help"]:
                print(
                    "Usage: ambulon md2html <input.md> [-o <output.html>] [--verbose] [--no-standalone] [--toc-backlinks]"
                )
                print()
                print(
                    "Convertit un fichier Markdown en HTML (SANS conversion des diagrammes)"
                )
                ...
                print("Options:")
                print(
                    "  -o, --output <file>           Fichier HTML de sortie (défaut: <input>.html)"
                )
                print(
                    "  --verbose                     Mode verbeux (affiche les détails)"
                )
                print(
                    "  --no-standalone               Génère un fragment HTML au lieu d'un document complet"
                )
                print(
                    "  --toc-backlinks               Ajoute des liens retour (↑) après chaque titre"
                )
                print()
                print("Exemples:")
            ...
            return process_markdown_to_html_simple(
                input_file, output_file, verbose, standalone, toc_backlinks
            )
            ...
            if len(sys.argv) < 3 or sys.argv[2] in ["-h", "--help"]:
                print(
                    "Usage: ambulon md2html-diagrams <input.md> [-o <output.html>] [-p <orientation>] [--verbose] [--no-standalone] [--toc-backlinks] [--plantuml-method auto|jar|kroki] [--plantuml-jar <path>]"
                )
                print()
                print(
                    "Convertit un fichier Markdown en HTML AVEC conversion des diagrammes en SVG"
                )
                ...
                print(
                    "  -p, --page-orientation <mode> Optimisation pour génération PDF:"
                )
                print(
                    "                                  portrait  - 700px max (A4 portrait)"
                )
                print(
                    "                                  landscape - 900px max (A4 landscape)"
                )
                print(
                    "                                  (par défaut: taille naturelle des SVG, aucune contrainte)"
                )
                print(
                    "  --verbose                     Mode verbeux (affiche les détails)"
                )
                ...
                print(
                    "                                  jar    - Utilise PlantUML.jar local (plus fiable)"
                )
                print(
                    "                                  kroki  - Utilise le service Kroki en ligne (défaut)"
                )
                print(
                    "  --plantuml-jar <path>         Chemin vers le fichier PlantUML.jar"
                )
                print()
                print("Exemples:")
                ...
                print(
                    "  ambulon md2html-diagrams document.md -p landscape --plantuml-method jar"
                )
                print()
                print("Note:")
                print(
                    "  Si vous avez des problèmes de connexion avec Kroki, utilisez --plantuml-method jar"
                )
                return 0 if len(sys.argv) > 2 and sys.argv[2] in ["-h", "--help"] else 1
            input_file = sys.argv[2]
            output_file = None
            ...
            while i < len(sys.argv):
                if sys.argv[i] in ["-o", "--output"] and i + 1 < len(sys.argv):
                    output_file = sys.argv[i + 1]
                    i += 2
                elif sys.argv[i] in ["-p", "--page-orientation"] and i + 1 < len(
                    sys.argv
                ):
            ...
            # Lancer le serveur webchat FastAPI intégré
            import argparse
            parser = argparse.ArgumentParser(prog="ambulon webchat", description="Lancer l'interface webchat RAG")
            ...
            parser.add_argument("--timeout", type=int, default=600, help="Timeout Ollama en secondes (défaut: 600)")
            parser.add_argument("--llm-model", help="Modèle LLM à utiliser (ex: deepseek-v4-flash:cloud)")
            args = parser.parse_args(sys.argv[2:])
            from app.webchat.commands.webchat_server import run_webchat_server
            return run_webchat_server(args)
        elif command == "html2pdf":
            # Convertir HTML en PDF
            from app.conversion.commands.html2pdf import (
                convert_html_to_pdf,
            ...
            if len(sys.argv) < 3 or sys.argv[2] in ["-h", "--help"]:
                print(
                    "Usage: ambulon html2pdf <input.html> [-o <output.pdf>] [-p <orientation>] [-m <method>] [--wkhtmltopdf-path <path>] [--verbose]"
                )
                print()
                ...
                print("Options:")
                print(
                    "  -o, --output <file>           Fichier PDF de sortie (défaut: <input>.pdf)"
                )
                print("  -p, --page-orientation <mode> Orientation de la page:")
                print(
                    "                                  portrait  - Format vertical (défaut)"
                )
                print("                                  landscape - Format horizontal")
                print(
                    "  -m, --method <method>         Méthode de rendu: auto, chromium, wkhtmltopdf"
                )
                print(
                    "  --wkhtmltopdf-path <path>     Chemin vers l'exécutable wkhtmltopdf"
                )
                print(
                    "  --verbose                     Mode verbeux (affiche les détails)"
                )
                print(
                    "  --install-chromium            Affiche le guide d'installation de Chromium"
                )
                print()
                print("Exemples:")
                ...
                print("Note:")
                print(
                    "  Pour un workflow optimal, utilisez la même orientation dans md2html et html2pdf:"
                )
            ...
            return convert_html_to_pdf(
                input_file, output_file, orientation, method, wkhtmltopdf_path, verbose
            )
            ...
            if len(sys.argv) < 3:
                print(
                    "Usage: ambulon json2jsonl <input.json> -o <output.jsonl> [--array-key KEY] [--verbose]"
                )
                return 1
            input_file = sys.argv[2]
            output_file = None
            ...
            if len(sys.argv) < 3:
                print(
                    "Usage: ambulon json2md <input.json> [-o <output.md>] [--verbose]"
                )
                return 1
            input_file = sys.argv[2]
            output_file = None
            ...
            if len(sys.argv) < 3:
                print(
                    "Usage: ambulon flatten-wikisi <source_dir> [-o <output_dir>] [--verbose]"
                )
                return 1
            source = sys.argv[2]
            output = None
            ...
            if len(sys.argv) < 3:
                print(
                    "Usage: ambulon augment <input.html> [-o <output.html>] [--verbose]"
                )
                return 1
            input_file = sys.argv[2]
            output = None
            ...
            from app.processing.commands.md_to_interactive_html import (
                main as md_to_interactive_main,
            )
