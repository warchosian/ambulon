...
from app.processing.commands.md_to_interactive_html import md_to_interactive_html
...
def generate_code_monofile(
    ...
    """Generate <repo>.code.md using project2md logic, plus augmented HTML."""
    ...
    # Convert to interactive HTML for each requested .html target
    for html_target in html_targets:
        # Use md2interactive to generate interactive HTML with TOC and backlinks
        exit_code = md_to_interactive_html(
            input_path=str(md_path),
            output_dir=str(base_dir),
            verbose=False
        )
        if exit_code == 0:
            # md2interactive produces: {base}-toced.md, {base}-itoced.md, {base}-itoced.html, {base}-augmented.html
            # Move/rename the augmented file to the desired html_target if needed
            augmented_file = base_dir / f"{md_path.stem}-augmented.html"
            if augmented_file.exists() and html_target.name != augmented_file.name:
                try:
                    augmented_file.rename(html_target)
                    logger.info(f"Augmented HTML created: {html_target}")
                except Exception as e:
                    logger.warning(f"Could not rename {augmented_file} to {html_target}: {e}")
                    logger.info(f"Augmented HTML available at: {augmented_file}")
            else:
                logger.info(f"Augmented HTML created: {augmented_file}")
        else:
            logger.error(f"Failed to generate augmented HTML for {md_path}")
            return 1, md_path
...
def generate_wiki_monofile(
    ...
    """Generate <repo>.md from a wiki repo using flatten-md + merge-md, plus augmented HTML."""
    base_dir = _resolve_base_dir(repo_dir, output_dir, output_mode)
    base_dir.mkdir(parents=True, exist_ok=True)
    templates_list = templates if templates is not None else [filename_template] if filename_template else None
    if not templates_list:
    ...
    for html_target in html_targets:
        ...
        if exit_code == 0:
            ...
            logger.error(f"Failed to generate interactive HTML for {md_path}")
            return 1, md_path
