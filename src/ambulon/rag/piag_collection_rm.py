...
"""Module pour supprimer une collection RAG PIAG."""
...
def delete_collection(
    ...
    """
    Supprime une collection RAG PIAG.

    Args:
        collection_id: L'identifiant de la collection RAG à supprimer.
    ...
    try:
        ...
        print(f"Erreur lors de la suppression de la collection: {e}", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Supprime une collection RAG PIAG.",
        epilog="HIÉRARCHIE DE PRIORITÉ: Arguments CLI > Fichier YAML > Variables d'environnement > Valeurs par défaut"
    )
    parser.add_argument("--collection-id", help="ID de la collection RAG à supprimer (écrase YAML et env).")
    ...
    if not args.confirm:
        print(f"⚠️  Vous êtes sur le point de supprimer la collection: {collection_id}")
    ...
    try:
        ...
        print(f"✅ Collection {collection_id} supprimée avec succès.")
