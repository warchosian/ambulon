...
# -*- coding: utf-8 -*-
"""
Serveur MCP Ambulon - Version Standalone avec OCR
==================================================

Cette version importe les outils fonctionnels incluant l'OCR
et contourne les problèmes d'imports du projet principal.

Usage:
    python mcp_server_standalone_with_ocr.py
"""
...
# Créer le serveur
server = Server("ambulon-standalone-ocr")

logger.info("=" * 70)
logger.info("SERVEUR MCP AMBULON - VERSION STANDALONE AVEC OCR")
logger.info("=" * 70)
...
# Test OCR
try:
    from app.ocr.core.ocr_logic import perform_ocr_single_file, _process_pdf_ocr
    tools_status['ocr'] = True
    logger.info("✓ OCR tools loaded")
except Exception as e:
    tools_status['ocr'] = False
    logger.warning(f"✗ OCR tools failed: {e}")
...
async def list_tools():
    ...
    if tools_status.get('ocr'):
        available_tools.append(
            Tool(
                name="ocr_document",
                description="Effectue l'OCR sur un fichier PDF ou image pour extraire le texte",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "input_file": {"type": "string", "description": "Chemin du fichier PDF ou image"},
                        "output_file": {"type": "string", "description": "Chemin du fichier texte de sortie (optionnel)"},
                        "language": {"type": "string", "description": "Langue pour l'OCR (fra, eng, etc.)", "default": "fra"}
                    },
                    "required": ["input_file"]
                }
            )
...
async def call_tool(name: str, arguments: dict):
    ...
    try:
        if name == "html_to_markdown":
            ...
            if not tools_status.get('ocr'):
                return [TextContent(
                    type="text",
                    text="OCR tools not available"
                )]

            from app.ocr.core.ocr_logic import perform_ocr_single_file, _process_pdf_ocr

            input_file = Path(arguments["input_file"])
            output_file = Path(arguments.get("output_file")) if arguments.get("output_file") else None
            language = arguments.get("language", "fra")

            # Déterminer si c'est un PDF ou une image
            if input_file.suffix.lower() == '.pdf':
                result = _process_pdf_ocr(input_file, language, output_file)
            else:
                result = perform_ocr_single_file(input_file, language, output_file)

            if result['success']:
                return [TextContent(
                    type="text",
                    text=f"OCR réussi: {result}"
                )]
            else:
                return [TextContent(
                    type="text",
                    text=f"Erreur OCR: {result.get('error', 'Erreur inconnue')}"
                )]
