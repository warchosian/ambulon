...
- **Tous les diagrammes architecturaux doivent suivre la notation C4** (personnes, systèmes logiciels, conteneurs, composants) avec la syntaxe Mermaid C4.
...
2. **Niveau 1 — Vue Contexte (System Context)**
   - **Diagramme C4-L1** en Mermaid : montre le système à architecturer au centre, les utilisateurs humains et les systèmes externes autour.
...
5. **Niveau 2 — Vue Conteneurs (Containers)**
   - **Diagramme C4-L2** en Mermaid : décompose le système en applications/conteneurs (applications web, mobiles, bases de données, fichiers, etc.).
...
6. **Niveau 3 — Vue Composants (Components)** *(optionnel selon complexité)*
   - **Diagramme C4-L3** en Mermaid pour 1 à 3 conteneurs principaux : décompose un conteneur en composants (contrôleurs, services, repositories, etc.).
...
8. **Vue Exécution (Scénarios)**
   ...
   - Utilise des **diagrammes de séquence Mermaid** ou une description textuelle claire.
   - Montre le flux traversant les différents niveaux C4.

9. **Vue Déploiement** *(section standardisée)*
   - **Diagramme C4-Déploiement** en Mermaid : montre l'allocation des conteneurs sur les infrastructures (serveurs, cloud, on-premise).
...
- **Tous les diagrammes architecturaux doivent utiliser la bibliothèque C4-Mermaid** avec l'inclusion :
  ```Mermaid
...
- Le document doit être **compatible** avec les extensions VS Code / Obsidian (ex. : Markdown Preview Enhanced, Mermaid).
...
- Prêt à être utilisé tel quel dans un environnement de documentation technique avec support C4-Mermaid activé.
...
- Standardisation sur la **syntaxe C4-Mermaid** avec inclusion des bibliothèques officielles
